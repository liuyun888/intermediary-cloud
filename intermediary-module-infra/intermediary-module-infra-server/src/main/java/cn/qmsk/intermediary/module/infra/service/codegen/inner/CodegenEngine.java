package cn.qmsk.intermediary.module.infra.service.codegen.inner;
...
import cn.qmsk.intermediary.framework.apilog.core.annotation.ApiAccessLog;
import cn.qmsk.intermediary.framework.apilog.core.enums.OperateTypeEnum;
...
import cn.qmsk.intermediary.framework.common.util.date.LocalDateTimeUtils;
...
import cn.qmsk.intermediary.framework.excel.core.annotations.DictFormat;
import cn.qmsk.intermediary.framework.excel.core.convert.DictConvert;
import cn.qmsk.intermediary.framework.excel.core.util.ExcelUtils;
...
import cn.qmsk.intermediary.module.infra.enums.codegen.CodegenVOTypeEnum;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 intermediary-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 intermediary-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("cn.qmsk.intermediary.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/intermediary-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/intermediary-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "intermediary-module-${table.moduleName}/" + // 顶级模块
                "intermediary-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "intermediary-module-${table.moduleName}/" + // 顶级模块
                "intermediary-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "intermediary-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "intermediary-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "intermediary-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "intermediary-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
